...
+ This analysis's findings suggest that the AMN analyses parameters need to be re-thought to match the times when, de facto, the evolution of AMN changes. 
...
+ The increased trend reflects the intensified programme response at the time, to fit the also increased needs.
...
+ **Riverine livelihood system**: 
    ...
    - The pattern in the riverine differ remarkably from the other systems. **It is the only system where the high-peak does not occur in May-June**
...
1. An insteresting finding is that the riverine and pastoral, are the only livelihood systems where there is a peak in March before the lower nadir in April.
**Why is that??** *Question to the Nutrition Cluster members.*

2. Where might the observed patterns stem from?
:::

## {.smaller}
### Seasonal Variation (3/3)
...
## {.smaller}
### Recommendations for further analysis following today's results-sharing session 

+ **What?** 
    - Understand the seasonal variation of the most common child morbidity.
    - Time series data is available through the FSNAU.
        - Data is reported monthly. 
    
+ **Action point**: 
    - Request access to the FSNAU -> run analysis -> incorporate findings into the report.
    - Split the analysis by livelihood systems as well. 
