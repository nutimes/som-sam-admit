...
```
  ...
  + **Post-Gu**: characterized by increased pasture and water availability for the livestock and increased availability of milk, fruits, vegetables. This is as well the peak season of a liquid butter made of milk know as *ghee*.  
  + **Hagaa**: a dry season, characterized by light rainfalls.  
  + **Deyr**: a rainy season, however localized and less reliable.

It is generally assumed that AMN in Somalia peaks from the end of the Jilaal season to the end of the Gu season. An analysis by @ENN, based on SMART surveys conducted by the Food Security and Nutrition Analysis Unit (FSNAU) and partners, with data collected between the Gu and the Deyr seasons from 2014 to 2021, identified two distinct seasonal peaks: **May/June (the beginning of Gu)** and **November (mid-Deyr)**, with a decline during the lean season. The study also highlighted variations of these patterns based on livelihood zones, including pastoralist, agro-pastoralist, and riverine/farming communities:
...
It consisted in excluding districts with missing admissions from January to December; then by aggregating the sum of the total number of children admitted by month at national level, and by livelihood systems (Pastoral, Agropastoral, Riverine and Urban/IDPs) as demonstrated in below flowchart.
```{mermaid}
...
The figure below displays the total number of SAM cases admitted into treatment programs in Somalia over six years, from January 2019 to December 2024. It highlights a shifting trend over time: a relatively stable period before 2022, followed by a significant rise, and then a consistent decline beginning in 2023. Additionally, the figure reveals the existence of a seasonal pattern (the ups and downs), however it seems to vary over time as well.
...
@fig-seasonal-plot-national displays the evolution of admissions across months (x-axis) by year (y-axis). Each line represents the evolution of the admissions throughout the months of a particular year. 
...
The majority of the districts in Somalia are pastoral (34 districts), followed by agropastoral (19 districts). @tbl-lsystem-districts counts districts after excluding those with missing values while @fig-livelihood-systems depicts the spatial distribution of the livelihood system over the map of Somalia.
...
All in all, the admissions in each livelihood system seem to follow the same pattern described in @sec-glimpse-national. This is further unpacked in the subsequent sections.
...
## How should Table 2 be read in conjunction with Figure 10?
...
###### Analyzing the trend component  
The direction of the trend has undergone six significant shifts, as illustrated below, with the ARC summarized in @tbl-arc-pastoral.
```{r}
...
###### Analyzing the trend component   
The direction of the trend has undergone four significant shifts, as illustrated below, with the ARC summarized in @tbl-arc-riverine.
```{r}
...
###### Analyzing the trend component  
The direction of the trend has undergone six significant shifts, as illustrated below, with the ARC summarized in @tbl-arc-riverine
```{r}
...
###### Analyzing the trend component  
The direction of the trend has undergone eight significant shifts, as illustrated below, with the ARC summarized in @tbl-arc-urban. 
...
: The average rate of change of admissions by month in the time-intervals represented in @fig-piecewise-slopes-urban-idps {#tbl-arc-urban}  
...
This analysis's findings suggest that the AMN analysis parameters need to be rethought to match the times when AMN evolution changes de facto. This should be sensitive to the existing livelihood systems. 

Below are the seasonal calendars of AMN by livelihood system, translated from the seasonal variation graphs presented in the previous sections of this report. The graphs depict the frequency and length of a particular AMN season denoted as "High" in red and "Low" in blue.

#### Pastoral livelihood system 

In this livelihood system, the high AMN season occurs three times, while the low occurs two times. 
...
In this livelihood system, both the high and the low AMN season occurs two times each.
...
In this livelihood system, both the high and the low AMN season occurs three times each.
...
All in all, the unveiled AMN seasonal patterns do not fully agree with the seasons of AFI, as it was believed to be, theoretically. This is true for the Jilaal season in particular, where most of its period falls under the AMN low season and the very small portion of the AMN high season is a carryover from the Deyr season that precedes it. It is also true for the Gu season during which food availability starts to increase, compared to the Jilaal season, nevertheless, the AMN observes its highest season. As for the Deyr season, it tends to align with the AMN's second high season in all livelihood systems. This relationship can be visualized in @fig-afi-amn-calendar-pastoral, @fig-afi-amn-calendar-agropastoral, @fig-afi-amn-calendar-riverine and @fig-afi-amn-calendar-urbanidps
...
gantt
    ...
    Peak :crit, ntl1, 1, 2
    Low nadir :ntl2, 2, 5
    High peak :crit, ntl3, 5, 7
    Nadir :ntl4, 7, 11
    Peak :crit, ntl5, 11, 12
...
gantt
    ...
    Low nadir :amn2, 3, 5
    High peak :amn3, 5, 9
    Lower nadir :amn5, 9, 11
    Higher peak :amn6, 11, 12
    section National Seasons
    Peak :crit, ntl1, 1, 2
    Low nadir :ntl2, 2, 5
    High peak :crit, ntl3, 5, 7
...
Regarding whether the observed AMN season follows the AFI's, the results show that they do not agree fully, except for the Deyr season. In the Jilaal and the Gu AFI seasons, AMN presents itself in the opposite direction of the AFI, raising questions on the other non-food security factors that play a remarkable role, either protecting children from getting malnourished amidst the lean season that characterizes the Jilaal - Somalia's extended dry season, with low milk availability and increased livestock migration in pastoralist, in particular -; or increasing the likelihood of a child getting malnourished amidst an increased food availability that characterizes the Gu season - Somalia's heavier rainfall, accounting for 70% of production. 
